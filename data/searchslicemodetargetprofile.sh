...
# Copy of the profile DB that can be reduced at every iteration the search
...
# $MAX_STEPS is set in the env by MMSeqs
STEP=0
...
do
    ...
    rm -f "$TMP_PATH/searchOut.new.sorted" "$TMP_PATH/searchOut.new.sorted.index"
    ...
    notExists "$TMP_PATH/searchOut.$offset.count" && "$MMSEQS" result2stats "$PROFILEDB" "$INPUT" "$TMP_PATH/pref_notSwapped.$offset" \
              "$TMP_PATH/searchOut.$offset.count"  --stat linecount $COMMONS
    notExists "$TMP_PATH/searchOut.$offset.toKeep" && "$MMSEQS" filterdb "$TMP_PATH/searchOut.$offset.count" "$TMP_PATH/searchOut.$offset.toKeep" \
                --filter-column 1 --comparison-operator ge --comparison-value "$MAX_SEQS" $COMMONS
