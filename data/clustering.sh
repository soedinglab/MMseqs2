...
notExists "$3/aln_redundancy" && $RUNNER $MMSEQS align $INPUT $INPUT "$3/pref_redundancy" "$3/aln_redundancy" "$3/clu_redundancy" ${ALIGNMENT_PAR} && checkReturnCode "Alignment step $STEP died"
...
if [ -n "$CLUSTER_FRAG" ]; then
    notExists "$3/clu" && $MMSEQS mergeclusters "$1" "$3/clu" "$3/clu_frag" "$3/clu_redundancy" "$3/clu_step0" && checkReturnCode "Merging of clusters has died"
else
...
if [ -n "$REMOVE_TMP" ]; then
 ...
 rm -f "$3/input_step_frag" "$3/input_step_frag.index"
