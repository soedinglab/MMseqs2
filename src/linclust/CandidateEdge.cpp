...
// The *sequence* bytes one edge bucket should span, when memory would allow a
// bigger one. Buckets are ranges of representative key, so this is the natural
// unit: it is what the align worker loads whole.
//
// It is a granularity target, not a memory bound. An align worker also holds that
// key range's candidate edges, which on the measured MGnify corpus run to roughly
// the same order as the sequence bytes on disk and expand further once decoded,
// so the resident set is a multiple of this figure. The memory *ceiling* below --
// memoryLimitBytes / 4 -- is what bounds that; this constant only stops the count
// collapsing when the ceiling is generous.
//
// Deliberately far below any plausible per-node budget, because this count is
// what bounds align-stage parallelism. At the measured 189 B/sequence, 1 GiB puts
// 1e8 sequences at 32 buckets and 1e9 at 256, which feeds any allocation those
// scales are run on, while staying large enough that the per-bucket write buffers
// EdgeBucketWriter carves out of a fixed budget are still worth having.
...
// Below this a "memory limit" is not a limit anyone meant, and honouring it would
// derive tens of thousands of buckets for a trivial input.
const uint64_t MIN_ALIGN_BUCKET_BYTES = 1ULL << 20;
}  // namespace
...
unsigned int deriveAlignBucketCount(uint64_t sequenceBytes, uint64_t entryCount,
    ...
    // A tiny or zero target would double all the way to the ceiling on any input
    // at all, which is a worse answer than ignoring an unusable limit. Guarded on
    // a floor rather than on zero so a nonsensical limit of a few bytes behaves
    // like an unset one instead of jumping to 65536 buckets.
    const uint64_t effectiveTarget = targetBucketBytes >= MIN_ALIGN_BUCKET_BYTES
                                         ? targetBucketBytes
                                         : TARGET_ALIGN_BUCKET_BYTES;
    while (bucketCount < MAX_ALIGN_BUCKETS && sequenceBytes / bucketCount > effectiveTarget) {
        bucketCount *= 2;
    }
    ...
    // sequences are ~1.9 GB, which is two buckets however many workers wait.
    if (workerCount > 0) {
