...
int kmerindexdb(int argc, const char **argv, const Command &command) {
    ...
    DBReader<KeyType> seqDbr(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    ...
    if (par.checkCompatible > 0 && FileUtil::fileExists(indexDB.c_str())) {
        ...
        DBReader<KeyType> index(indexDB.c_str(), (indexDB + ".index").c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
        index.open(DBReader<KeyType>::NOSORT);
    ...
    size_t writePos = 0;
    size_t mpiRank = 0;
    int adjustedKmerSize = par.kmerSize;
#ifdef HAVE_MPI
    ...
    if(mpiRank == 0){
        ...
        DBReader<KeyType> dbr1(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
        dbr1.open(DBReader<KeyType>::NOSORT);
        ...
        dbw.writeData(data, DBReader<KeyType>::indexMemorySize(dbr1), PrefilteringIndexReader::DBR1INDEX, 0);
        dbw.alignToPageSize();
        ...
        if (sameDB == true) {
            dbw.writeIndexEntry(PrefilteringIndexReader::DBR2INDEX, offsetIndex, DBReader<KeyType>::indexMemorySize(dbr1) + 1, 0);
            dbw.writeIndexEntry(PrefilteringIndexReader::DBR2DATA,  offsetData,  dbr1.getTotalDataSize()+1, 0);
            dbr1.close();
        }else{
            dbr1.close();
            DBReader<KeyType> dbr2(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
            dbr2.open(DBReader<KeyType>::NOSORT);
            Debug(Debug::INFO) << "Write DBR2INDEX (" << PrefilteringIndexReader::DBR2INDEX << ")\n";
            data = DBReader<KeyType>::serialize(dbr2);
            dbw.writeData(data, DBReader<KeyType>::indexMemorySize(dbr2), PrefilteringIndexReader::DBR2INDEX, 0);
            dbw.alignToPageSize();
...

        {
            ...
            DBReader<KeyType> hdbr1(par.hdr1.c_str(), par.hdr1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
            hdbr1.open(DBReader<KeyType>::NOSORT);

            data = DBReader<KeyType>::serialize(hdbr1);
            size_t offsetIndex = dbw.getOffset(0);
            dbw.writeData(data, DBReader<KeyType>::indexMemorySize(hdbr1), PrefilteringIndexReader::HDR1INDEX, 0);
            dbw.alignToPageSize();
            ...
            free(data);
            if (sameDB == true) {
                dbw.writeIndexEntry(PrefilteringIndexReader::HDR2INDEX, offsetIndex, DBReader<KeyType>::indexMemorySize(hdbr1) + 1, 0);
                dbw.writeIndexEntry(PrefilteringIndexReader::HDR2DATA,  offsetData, hdbr1.getTotalDataSize()+1, 0);
                hdbr1.close();
            }else{
                hdbr1.close();
                DBReader<KeyType> hdbr2(par.hdr2.c_str(), par.hdr2Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
                hdbr2.open(DBReader<KeyType>::NOSORT);
                Debug(Debug::INFO) << "Write HDR2INDEX (" <<PrefilteringIndexReader::HDR2INDEX << ")\n";
                data = DBReader<KeyType>::serialize(hdbr2);
                dbw.writeData(data, DBReader<KeyType>::indexMemorySize(hdbr2), PrefilteringIndexReader::HDR2INDEX, 0);
                dbw.alignToPageSize();
