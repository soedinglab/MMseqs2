...
void KmerGenerator::calcDivideStrategy(){
    ...
    switch(kmerSize%3){
        case 0:
            ...
            for(int i = 0; i < threeDivideCount; i++){
                this->divideStep[i] = 3;
        ...
        case 1: 
            ...
            this->divideStep = new unsigned int[divideStepCount];
            for(int i = 0; i < threeDivideCount-1; i++){
                this->divideStep[i] = 3;
                this->matrixLookup[i] = three;
            }
...
void KmerGenerator::initResultList(size_t divide_steps){
    ...
    for(size_t i = 0 ; i < divide_steps; i++){
        ...
        for(size_t j = 0 ; j < VEC_LIMIT; j++){
            outputArray[i][j]= new std::pair<short, unsigned int>();
        }
    }
}
...
KmerGeneratorResult KmerGenerator::generateKmerList(const int * int_seq){
    ...
    for(int i = 0; i < this->divideStepCount; i++){
        ...
        const std::pair<short,unsigned int> * score=extMatrix->scoreMatrix[index][0];
    ...
    for(i = 0; i < this->divideStepCount-1; i++){
        ...
        size_t lastElm=calculateArrayProduct(inputArray,
                                   sizeExtendedMatrix,
                                   nextIndexScoreArray,
                                   sizeExtendedMatrix,
                                   outputArray[i],
