...
void parseArgs(int argc, const char** argv, std::string* ffindexQueryDBBase, std::string* ffindexTargetDBBase, std::string* scoringMatrixFile, std::string* ffindexOutDBBase, float* prefThr, int* kmerSize, int* alphabetSize){
    if (argc < 5){
        printUsage();
        exit(EXIT_FAILURE);
    }
...
int main (int argc, const char * argv[])
{
    float prefThr = 0.55f;
    int kmerSize =  6;
    short kmerThr =  (short)(4.5 * kmerSize);
    int alphabetSize = 21;
    ...
    parseArgs(argc, argv, &queryDB, &targetDB, &scoringMatrixFile, &outDB, &prefThr, &kmerSize, &alphabetSize);

    std::cout << "Prefiltering threshold: " << prefThr << "\n";
    std::cout << "k-mer size: " << kmerSize << "\n";
    std::cout << "Alphabet size: " << alphabetSize << "\n";
    std::cout << "Kmer similarity threshold: " << kmerThr << "\n\n";
