...
Prefiltering::Prefiltering(const std::string &queryDB,
    ...
    if (Parameters::isEqualDbtype(FileUtil::parseDbType(targetDB.c_str()), Parameters::DBTYPE_INDEX_DB)) {
        ...
        tidxdbr = new DBReader<IdType>(targetDB.c_str(), targetDBIndex.c_str(), threads, DBReader<IdType>::USE_INDEX | DBReader<IdType>::USE_DATA);
        ...
        tdbr = new DBReader<IdType>(targetDB.c_str(), targetDBIndex.c_str(), threads, DBReader<IdType>::USE_INDEX|DBReader<IdType>::USE_DATA);
        tdbr->open(DBReader<IdType>::LINEAR_ACCCESS);
    ...
    if (templateDBIsIndex == false && sameQTDB == true) {
        ...
        qdbr = new DBReader<IdType>(queryDB.c_str(), queryDBIndex.c_str(), threads, DBReader<IdType>::USE_INDEX|DBReader<IdType>::USE_DATA);
    ...
    if(Parameters::isEqualDbtype(targetSeqType, Parameters::DBTYPE_NUCLEOTIDES) == false){
        const bool isProfileSearch = Parameters::isEqualDbtype(querySeqType, Parameters::DBTYPE_HMM_PROFILE) ||
                                     Parameters::isEqualDbtype(targetSeqType, Parameters::DBTYPE_HMM_PROFILE);
        const bool queryCPC = DBReader<IdType>::getExtendedDbtype(querySeqType) & Parameters::DBTYPE_EXTENDED_CONTEXT_PSEUDO_COUNTS;
        const bool targetCPC = DBReader<IdType>::getExtendedDbtype(targetSeqType) & Parameters::DBTYPE_EXTENDED_CONTEXT_PSEUDO_COUNTS;
...
void Prefiltering::setupSplit(DBReader<IdType>& tdbr, const int alphabetSize, const unsigned int querySeqTyp, const int threads,
...
void Prefiltering::mergeTargetSplits(const std::string &outDB, const std::string &outDBIndex, const std::vector<std::pair<std::string, std::string>> &fileNames, unsigned int threads) {
    ...
    if (splits < 2) {
        DBReader<IdType>::moveDb(fileNames[0].first, outDB);
    ...
    DBReader<IdType> reader1(fileNames[0].first.c_str(), fileNames[0].second.c_str(), 1, DBReader<IdType>::USE_INDEX);
    reader1.open(DBReader<IdType>::NOSORT);
    DBReader<IdType>::Index *index1 = reader1.getIndex();
    ...
    for (size_t i = 1; i < splits; ++i) {
        DBReader<IdType> reader2(fileNames[i].first.c_str(), fileNames[i].second.c_str(), 1, DBReader<IdType>::USE_INDEX);
        reader2.open(DBReader<IdType>::NOSORT);
        DBReader<IdType>::Index *index2 = reader2.getIndex();
        size_t currOffset = 0;
        for (size_t id = 0; id < reader1.getSize(); id++) {
...
#pragma omp parallel num_threads(threads)
    ...
    for (size_t i = 0; i < splits; ++i) {
        DBReader<IdType>::removeDb(fileNames[i].first);
...
void Prefiltering::runMpiSplits(const std::string &resultDB, const std::string &resultDBIndex, const std::string &localTmpPath, const int runRandomId) {
    ...
    if (localTmpPath != "") {
        ...
        DBReader<IdType>::moveDb(result.first, resultShared.first);
    }
    ...
    if (splitProcessCount > 1) {
        ...
        if (splitFiles.size() > 0) {
            ...
            if (splitFiles.size() > 1) {
                DBReader<IdType> resultReader(resultDB.c_str(), resultDBIndex.c_str(), threads, DBReader<IdType>::USE_INDEX | DBReader<IdType>::USE_DATA);
                ...
                resultReader.close();
                DBReader<IdType>::removeDb(resultDB);
                DBReader<IdType>::moveDb(tempDb.first, resultDB);
            }
            hasResult = true;
        }
...
#pragma omp for schedule(dynamic, 1) reduction (+: kmersPerPos, resSize, dbMatches, doubleMatches, querySeqLenSum, diagonalOverflow)
    ...
    if (splitMode == Parameters::TARGET_DB_SPLIT && splits > 1) {
        ...
        DBReader<IdType> resultReader(tmpDbw.getDataFileName(), tmpDbw.getIndexFileName(), threads, DBReader<IdType>::USE_INDEX|DBReader<IdType>::USE_DATA);
        resultReader.open(DBReader<IdType>::NOSORT);
        resultReader.readMmapedDataInMemory();
        const std::pair<std::string, std::string> tempDb = Util::databaseNames((resultDB + "_tmp"));
...
size_t Prefiltering::estimateMemoryConsumption(int split, size_t dbSize, size_t resSize,
    ...
    size_t dbReaderSize = dbSize * (sizeof(DBReader<IdType>::Index) + sizeof(unsigned int)); // DB index size
...
std::pair<int, int> Prefiltering::optimizeSplit(size_t totalMemoryInByte, DBReader<IdType> *tdbr,
