...
const bool tsvOut, const std::string &mappingFile, const std::string &userStrToAdd, const bool isPrefix, const int threads, const int compressed) {
    DBReader<DBKeyType> reader(db1.c_str(), db1Index.c_str(), threads, DBReader<DBKeyType>::USE_INDEX|DBReader<DBKeyType>::USE_DATA);
    ...
    if(mappingFile.size() > 0){
        lookupReader = new DBReader<DBKeyType>(mappingFile.c_str(), mappingFile.c_str(), 1, DBReader<DBKeyType>::USE_LOOKUP);
        doMapping = true;
    }
