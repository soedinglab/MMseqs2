...
int convertca3m(int argc, const char **argv, const Command &command) {
    ...
    DBReader<std::string> reader((par.db1 + "_ca3m.ffdata").c_str(), (par.db1 + "_ca3m.ffindex").c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    reader.open(DBReader<std::string>::NOSORT);

    DBReader<KeyType> sequences((par.db1 + "_sequence.ffdata").c_str(), (par.db1 + "_sequence.ffindex").c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    sequences.open(DBReader<KeyType>::SORT_BY_LINE);
