...
int mergeclusters(int argc, const char **argv, const Command &command) {
    ...
    DBReader<KeyType> dbr(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX);
    ...
    DBReader<KeyType> cluDb(firstClu.c_str(), firstCluStepIndex.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    cluDb.open(DBReader<KeyType>::LINEAR_ACCCESS);
...
#pragma omp for schedule(dynamic, 100)
        for (size_t i = 0; i < cluDb.getSize(); i++) {
            ...
            KeyType clusterId = cluDb.getDbKey(i);
            KeyType cluId = dbr.getId(clusterId);
            ...
            while (*data != '\0') {
                ...
                KeyType key = Util::fast_atoi<KeyType>(keyBuffer);
                KeyType seqId = dbr.getId(key);
    ...
    while (!clusterings.empty()) {
        ...
        DBReader<KeyType> cluDb(cluStep.c_str(), cluStepIndex.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
        cluDb.open(DBReader<KeyType>::LINEAR_ACCCESS);
...
#pragma omp for schedule(dynamic, 100)
            for (size_t i = 0; i < cluDb.getSize(); i++) {
                ...
                KeyType cluId = dbr.getId(cluDb.getDbKey(i));
                char *data = cluDb.getData(i, thread_idx);
                while (*data != '\0') {
                    Util::parseKey(data, keyBuffer);
                    KeyType key = Util::fast_atoi<KeyType>(keyBuffer);
