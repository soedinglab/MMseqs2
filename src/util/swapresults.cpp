    ...
    for (size_t i = 0; i < resultSize; ++i) {
        ...
        *(tmpBuff) = '\0';
        ...
        while (*data != '\0') {
            ...
            const unsigned int dbKey = Util::fast_atoi<unsigned int>(dbKeyBuffer);
...
#pragma omp parallel for schedule(dynamic, 100) reduction(+:bytesToWrite)
...
#pragma omp parallel for schedule(dynamic, 10)
    for (size_t i = 0; i < resultSize; ++i) {
        ...
        *(tmpBuff) = '\0';
        ...
        while (*data != '\0') {
            ...
            const unsigned int dbKey = (unsigned int) Util::fast_atoi<unsigned int>(dbKeyBuffer);
            size_t offset = __sync_fetch_and_add(&(targetElementSize[dbKey]), newLineLen);
    ...
    char *entry[255];
    const size_t columns = Util::getWordsOfLine(resultDbr.getData(0), entry, 255);
    bool isAlignmentResult = columns >= Matcher::ALN_RES_WITH_OUT_BT_COL_CNT;
    bool hasBacktrace = columns >= Matcher::ALN_RES_WITH_BT_COL_CNT;
...
#pragma omp parallel for schedule(dynamic, 100)
    for (size_t i = 0; i <= maxTargetId; ++i) {
        ...
        std::string ss;
        ss.reserve(100000);
        char *data = &tmpData[targetElementSize[i]];
