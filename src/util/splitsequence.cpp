...
int splitsequence(int argc, const char **argv, const Command& command) {
    ...
    if (par.headerSplitMode == 1) {
        headerReader = new DBReader<unsigned int>(par.hdr1.c_str(), par.hdr1Index.c_str(), par.threads, DBReader<unsigned int>::USE_INDEX|DBReader<unsigned int>::USE_DATA);
...
#pragma omp parallel
    {
        ...
        for (unsigned int i = queryFrom; i < (queryFrom + querySize); ++i) {
            ...
            if (par.headerSplitMode == 1) {
                ...
                headerLen = headerReader->getSeqLen(i);
            }
            ...
            for (size_t split = 0; split < splitCnt; split++) {
                ...
                if (par.headerSplitMode == 0) {
                    size_t fromPos = startPos;
                    size_t toPos = startPos + (len - 1);
                    size_t bufferLen = Orf::writeOrfHeader(buffer, key, fromPos, toPos, 0, 0);
