...
int appenddbtoindex(int argc, const char **argv, const Command &command) {
    ...
    size_t offset = 0;
    {
        DBReader<KeyType> outReader(outDb.c_str(), outIndexName.c_str(), 1, DBReader<KeyType>::USE_DATA | DBReader<KeyType>::USE_INDEX);
        outReader.open(DBReader<KeyType>::NOSORT);
    ...
    for (size_t i = 0; i < par.filenames.size(); ++i) {
        ...
        DBReader<KeyType> reader(inDb.c_str(), inIndexName.c_str(), 1, DBReader<KeyType>::USE_DATA | DBReader<KeyType>::USE_INDEX);
        reader.open(DBReader<KeyType>::HARDNOSORT);

        char* data = DBReader<KeyType>::serialize(reader);
        size_t inSize = DBReader<KeyType>::indexMemorySize(reader);
