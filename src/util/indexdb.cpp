...
std::string findIncompatibleParameter(DBReader<KeyType>& index, const Parameters& par, int kmerScore, const int dbtype) {
...
int indexdb(int argc, const char **argv, const Command &command) {
    ...
    DBReader<KeyType> dbr(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    ...
    DBReader<KeyType> *dbr2 = NULL;
    if ((sameDB == false) || ppDB) {
        dbr2 = new DBReader<KeyType>(db2.c_str(), db2Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    ...
    const bool contextPseudoCnts = DBReader<KeyType>::getExtendedDbtype(dbr.getDbtype()) & Parameters::DBTYPE_EXTENDED_CONTEXT_PSEUDO_COUNTS;
    ...
    if (recreate) {
        DBReader<KeyType> *hdbr1 = NULL;
        if (noHeaders == false) {
            hdbr1 = new DBReader<KeyType>(hdr1.c_str(), hdr1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
            hdbr1->open(DBReader<KeyType>::NOSORT);
        }

        DBReader<KeyType> *hdbr2 = NULL;
        if (sameDB == false && ppDB == false && noHeaders == false) {
            hdbr2 = new DBReader<KeyType>(par.hdr2.c_str(), par.hdr2Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
            hdbr2->open(DBReader<KeyType>::NOSORT);
        }

        DBReader<KeyType> *alndbr = NULL;
        const bool noAlignment = (par.indexSubset & Parameters::INDEX_SUBSET_NO_ALIGNMENT) != 0;
        if (ppDB == true && noAlignment == false) {
            alndbr = new DBReader<KeyType>(alnFile.c_str(), alnIndexFile.c_str(),
                                           par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
            alndbr->open(DBReader<KeyType>::NOSORT);
        }

        DBReader<KeyType>::removeDb(indexDB);
