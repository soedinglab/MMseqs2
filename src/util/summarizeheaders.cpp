...
int summarizeheaders(int argc, const char **argv, const Command& command) {
    ...
    DBReader<KeyType> queryReader(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    queryReader.open(DBReader<KeyType>::NOSORT);

    DBReader<KeyType> targetReader(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
    targetReader.open(DBReader<KeyType>::NOSORT);

    DBReader<KeyType> reader(par.db3.c_str(), par.db3Index.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
