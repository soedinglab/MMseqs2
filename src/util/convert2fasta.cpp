...
int convert2fasta(int argc, const char **argv, const Command& command) {
    ...
    DBReader<IdType> db(par.db1.c_str(), par.db1Index.c_str(), 1, DBReader<IdType>::USE_DATA|DBReader<IdType>::USE_INDEX);
    db.open(DBReader<IdType>::NOSORT);

    DBReader<IdType> db_header(par.hdr1.c_str(), par.hdr1Index.c_str(), 1, DBReader<IdType>::USE_DATA|DBReader<IdType>::USE_INDEX);
    db_header.open(DBReader<IdType>::NOSORT);
    ...
    for(size_t i = 0; i < from->getSize(); i++){
        IdType key = from->getDbKey(i);
        IdType headerKey = db_header.getId(key);
