...
int alignall(int argc, const char **argv, const Command &command) {
    ...
    DBReader<KeyType> tdbr(par.db1.c_str(), par.db1Index.c_str(), par.threads, DBReader<KeyType>::USE_DATA | DBReader<KeyType>::USE_INDEX);
    ...
    DBReader<KeyType> dbr_res(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<KeyType>::USE_DATA | DBReader<KeyType>::USE_INDEX);
...
#pragma omp for schedule(dynamic, 1)
            for (size_t id = start; id < (start + bucketSize); id++) {
                ...
                const KeyType key = dbr_res.getDbKey(id);
                ...
                for (size_t entryIdx1 = 0; entryIdx1 < results.size(); entryIdx1++) {
                    const KeyType queryId = tdbr.getId(results[entryIdx1]);
                    const KeyType queryKey = tdbr.getDbKey(queryId);
                    ...
                    *(tmpBuff-1) = '\t';
                    ...
                    for (size_t entryIdx = 0; entryIdx < results.size(); entryIdx++) {
                        const KeyType targetId = tdbr.getId(results[entryIdx]);
                        const KeyType targetKey = tdbr.getDbKey(targetId);
