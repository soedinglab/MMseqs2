...
int filterdb(int argc, const char **argv, const Command &command) {
    ...
    if (par.sortEntries != 0) {
        ...
        helper = new DBReader<KeyType>(par.joinDB.c_str(), joinIndex.c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
        helper->open(DBReader<KeyType>::NOSORT);
...
#pragma omp for schedule(dynamic, 10)
        for (size_t id = 0; id < reader.getSize(); ++id) {
            ...
            KeyType queryKey = reader.getDbKey(id);
