...
int gff2db(int argc, const char **argv, const Command &command) {
    ...
    DBReader<KeyType> reader(seqDb.c_str(), (seqDb + ".index").c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA | DBReader<KeyType>::USE_LOOKUP_REV);
    reader.open(DBReader<KeyType>::NOSORT);
    DBReader<KeyType> headerReader((seqDb + "_h").c_str(), (seqDb + "_h.index").c_str(), par.threads, DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA);
