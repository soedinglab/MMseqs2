...
int createsubdb(int argc, const char **argv, const Command& command) {
    ...
    int dbMode = DBReader<KeyType>::USE_INDEX | DBReader<KeyType>::USE_DATA;
    if (lookupMode) {
        dbMode |= DBReader<KeyType>::USE_LOOKUP_REV;
    }
    DBReader<KeyType> reader(par.db2.c_str(), par.db2Index.c_str(), 1, dbMode);
    ...
    while (getline(&line, &len, orderFile) != -1) {
        ...
        prevKey = key;
        const KeyType id = reader.getId(key);
        if (id >= UINT_MAX) {
    ...
    DBReader<KeyType>::softlinkDb(par.db2, par.db3, DBFiles::SEQUENCE_ANCILLARY);
