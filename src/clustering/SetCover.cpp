    ...
    do{ // for all elements in set
        ...
        for(unsigned int i = 0; i < array_size; i++){
            ...
            if(parent_set != NULL && parent_set->set_id != s_set_id){
                ...
                //A => A,B,C
                //B => D
                if(parent_set->set_id == element_to_remove->element_id){
                    parent_set->weight = 0;
                    set::element * elementToDelet=parent_set->elements;
                    do{
                        elementToDelet->parent_set = NULL;
                    } while((elementToDelet=elementToDelet->next) != NULL);
