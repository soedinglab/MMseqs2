...
Parameters::Parameters():
        PARAM_S(PARAM_S_ID,"-s", "Sensitivity","[int]\tSensitivity in the range [1:10]. From low (1) to high (10) sensitivity.", typeid(int), (void *) &sensitivity),
        ...
        PARAM_Z_SCORE(PARAM_Z_SCORE_ID,"--z-score", "Z-Score threshold", "[float]\tZ-score threshold (only implemented in global search mode)",typeid(float),(void *) &zscoreThr ),
        ...
        PARAM_SPLIT(PARAM_SPLIT_ID,"--split", "Split DB", "[int]\tSplits target set in n equally distributed chunks",typeid(int),(void *) &split),
        PARAM_SPLIT_MODE(PARAM_SPLIT_MODE_ID,"--split-mode", "Split mode", "[int]\tMPI Option: Target set: 0 (low memory) or query set: 1 (faster but memory intensiv)",typeid(int),(void *) &splitMode),
        ...
        PARAM_SPACED_KMER_MODE(PARAM_SPACED_KMER_MODE_ID,"--spaced-kmer-mode", "Spaced Kmer", "[int]\tSpaced kmers mode (use consecutive pattern). Disable: 0, Enable: 1",typeid(int), (void *) &spacedKmer ),
...
// clustering workflow
        ...
        PARAM_STEP(PARAM_STEP_ID, "--step","Step","[int]\t\tRestart the step of the cascaded clustering. For values in [1:3], the resprective step number, 4 is only the set merging",typeid(int),(void *) &step),
// search workflow
